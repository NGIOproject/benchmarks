...
double elapsed_time_bw_hr(struct timespec t1, struct timespec t2, int repeats, long int array_mbytes, char * title){
  ...
  elapsed_duration = elapsed_duration/repeats;

  printf("\n--- %s\n", title);
  printf("--- Timings ------------------------------------------------------------------------\n");
  printf("|\n");
  printf("| Test repeat %d time\n", repeats);
  printf("| Start: %.9lf   ", elapsed_start);
  printf("End: %.9lf   ", elapsed_end);
  printf("Duration (average of repeats): %.9lf s\n", elapsed_duration);
  printf("Bandwidth %.9lf MB/s\n", array_mbytes/elapsed_duration);
  printf("|\n");
  printf("------------------------------------------------------------------------------------\n");

  return elapsed_duration; // Compatibility
}
