...
#ifdef PMEM
  while((c = getopt_long(argc, argv, "b:s:r:o:d:a:l:ih", option_list, NULL)) != -1){
#else
...
#ifdef PMEM
  bench_level1(bench, size, rep, op, dt, algo, pmem_loc);
#else
...
#ifdef PMEM
  printf("\t -b, --bench NAME \t NAME of the benchmark - possible values are blas_op, stencil, fileparse, pmem_fileparse and cg.\n");
#else
